...
from model_lifecycle.retraining import run_retraining_request

DEFAULT_MONITORING_CONFIG_PATH = "configs/monitoring/global_monitoring.yaml"
...
logger = get_logger("evaluation.model.drift.drift")
...
def load_dataset(path: str | Path) -> pd.DataFrame:
    ...
    if dataset_path.suffix in {".jsonl", ".ndjson"}:
        return load_json_lines_dataset(dataset_path)

    raise ValueError(f"Formato de dataset não suportado para drift: {dataset_path}")


def load_json_lines_dataset(path: str | Path) -> pd.DataFrame:
    """Carrega JSONL validando linha a linha para diagnósticos claros."""
    ...
    with open(dataset_path, encoding="utf-8") as file_obj:
        for line_number, raw_line in enumerate(file_obj, start=1):
            ...
            try:
                ...
                raise ValueError(
                    f"JSONL inválido em {dataset_path}, linha {line_number}: "
                    f"{exc.msg}. Verifique se há registros colados sem quebra "
                    "de linha."
                ) from exc
            ...
            if not isinstance(record, dict):
                raise ValueError(
                    f"JSONL inválido em {dataset_path}, linha {line_number}: "
                    "cada linha deve conter um objeto JSON."
                )

            records.append(record)

    if not records:
        raise ValueError(f"Dataset JSONL de monitoramento está vazio: {dataset_path}")
    ...
    return [
        ...
        *cards,
        ...
        (
            ...
            "Fonte operacional: "
            "<code>configs/monitoring/global_monitoring.yaml</code>. "
