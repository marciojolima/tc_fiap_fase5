...
def test_build_feature_store_export_dataframe_reuses_pipeline(
    ...
    raw_df.insert(0, "CustomerId", range(1000, 1000 + len(raw_df)))
    raw_df.insert(0, "Surname", [f"Cliente{i}" for i in range(len(raw_df))])
    raw_df.insert(0, "RowNumber", range(1, len(raw_df) + 1))

    feature_input = raw_df.drop(columns=["RowNumber", "CustomerId", "Surname", "Exited"])
    ...
    assert export_df[FEATURE_ENTITY_JOIN_KEY].tolist()[0] == 1000
