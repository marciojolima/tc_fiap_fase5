...
def test_claude_ragas_factory_removes_conflicting_top_p(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "evaluation.ragas_eval.resolve_llm_api_key",
        lambda *_args, **_kwargs: "fake-key",
    )
    monkeypatch.setattr(
        "evaluation.ragas_eval.resolve_llm_base_url",
        lambda *_args, **_kwargs: None,
    )
    monkeypatch.setattr(
        "evaluation.ragas_eval.resolve_llm_max_tokens",
        lambda *_args, **_kwargs: _FAKE_MAX_TOKENS,
    )
    monkeypatch.setattr("evaluation.ragas_eval.llm_factory", fake_llm_factory)
